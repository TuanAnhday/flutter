...
class _CourseViewState extends State<CourseView> {
  ...
  void initState() {
    super.initState();
    _cubit
        .getAdmissionByStudentId(Application.sharePreference.getString('studentId')!)
        .whenComplete(() => _cubit.getCoursesHaveTrainingCenterWithStudent(Application.sharePreference.getString('studentId')!));
  }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: staticAppbar(
        ...
        title: Text('Khoá học', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15.sp)),
      ...
      body: Container(
        ...
        child: Column(
          children: [
            SizedBox(height: 1.h),
            Text('Danh sách khoá học', style: TextStyle(fontWeight: FontWeight.w600, fontSize: 15.sp, color: const Color(0xFF1B6086))),
            SizedBox(height: 1.h),
            FormBuilder(
              key: _formKey,
              child: FormBuilderTextField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: const Icon(Icons.search, color: Color(0xFF2F4D87), size: 24),
                ),
                onChanged: (val) {
                  final params = {'keyword': val};
                  Future.delayed(const Duration(milliseconds: 500),
                      () => _cubit.getCoursesHaveTrainingCenterWithStudent(Application.sharePreference.getString('studentId')!, params: params));
                },
  ...
  Widget buildBody() {
    return BlocBuilder(
      ...
      builder: (context, state) => state is CourseLoaded ? Expanded(
            child: SingleChildScrollView(
              child: Column(
                children: List.generate(_cubit.courses.length, (index) => Card(
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
                    child: TextButton(
                      style: TextButton.styleFrom(
                          padding: EdgeInsets.zero,
                          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
                          backgroundColor: const Color.fromRGBO(234, 247, 254, 1)),
                      onPressed: () {
                        Modular.to.pushNamed(
                          AppModule.main + NavBarModule.course + CourseModule.courseDetail,
                          ...
                          children: [
                            SizedBox(
                              ...
                              child: Padding(
                                ...
                                child: Image.asset('assets/images/home/course/course.png', width: 30.sp),
                              ),
                            ),
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    'Trạng thái: ${getClassStatus(_cubit.admissions
                                        .firstWhere((element) => element.courseId == _cubit.courses.elementAt(index).id, orElse: () => Admission()).classData?.classStatus)} ',
                                    style: TextStyle(fontSize: 9.sp, color: const Color(0xff84888B)),
                                  ),
                                  const SizedBox(height: 3),
                                  Text('Thời lượng: ${_cubit.courses.elementAt(index).numberOfSession}  buổi',
                                      style: TextStyle(fontSize: 9.sp, color: const Color(0xff84888B))),
                                ],
                              ),
                            ),
